...
const double _kAppBarMidHeight = 170.0;
...
class MyFavoriteListComponent extends StatefulWidget {
  const MyFavoriteListComponent({Key key}) : super(key: key);

  static const String routeName = '/animation';

  @override
  _MyFavoriteListComponentState createState() => _MyFavoriteListComponentState();
}

class _MyFavoriteListComponentState extends State<MyFavoriteListComponent> {
  ...
  void dispose() {
    ...
    _headingPageController.dispose();
                  ...
                  child: SizedBox(
                    ...
                    child: NotificationListener<ScrollNotification>(
                      ...
                      child: PageView(
                        ...
                        children: <Widget>[
                          UserFavoriteComponent(tabIndex: 0),
                          UserFavoriteComponent(tabIndex: 1),
                          UserFavoriteComponent(tabIndex: 2),
                          UserFavoriteComponent(tabIndex: 3),
                        ],
                      ),
                    ),
                  ),
          ...
          Positioned(
            ...
            child: Row(
              children: <Widget>[
                ...
                Text('收藏夹', style: TextStyle(color: Colors.white, fontSize: Theme.of(context).textTheme.subhead.height))
              ],
            ),
          ),
        ],
...
class UserFavoriteComponent extends StatefulWidget {
  final int tabIndex;
  UserFavoriteComponent({Key key, this.tabIndex = 0}) : super(key: key);

  _UserFavoriteComponentState createState() => _UserFavoriteComponentState();
}

class _UserFavoriteComponentState extends State<UserFavoriteComponent> with AutomaticKeepAliveClientMixin {
  int page;
  List _result;
  List pathName = ["getMyFavoriteCompany", "getMyFavoriteMission", "getMyFavoriteSkill", "getMyFavoriteArticle"];
  ...
  void initUserData() async {
    ...
    await Future.delayed(Duration(milliseconds: 600), () async {
      var res = await getMyFavoriteList(params: {"page": page}, pathName: pathName[widget.tabIndex]);
      if (null != res) {
        _result = res;
      }
    });
  ...
  Future<void> _onRefresh() async {
    ...
    await Future.delayed(Duration(milliseconds: 600), () {
      getMyFavoriteList(params: requestParams, pathName: pathName[widget.tabIndex]).then((v) {
        if (null != v) {
  ...
  Future<void> _onLoading() async {
    page += 1;
    var requestParams = {"page": page};
    await getMyFavoriteList(params: requestParams, pathName: pathName[widget.tabIndex]).then((v) {
      if (null != v) {
        setState(() {
          _result.addAll(v);
        });
        ...
        child: _result == null
            ...
            : _result.length == 0
                ...
                : ListView.builder(
                    ...
                    itemBuilder: _biuldMyFavoriteList,
                  ),
      ),
    );
  }

  Widget _biuldMyFavoriteList(BuildContext context, int index) {
    return Card(
      child: Stack(
        children: <Widget>[
          Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                Container(
                  child: Text(_result[index]['post_title'], overflow: TextOverflow.ellipsis, maxLines: 1),
                ),
                Padding(
                  ...
                  child: Row(
                    children: <Widget>[
                      ...
                      Text('${_result[index]['post_hits']}次浏览',
          ...
          Positioned.fill(
            child: Material(
              ...
              child: InkWell(
                ...
                onTap: () {
                  Navigator.pushNamed(context, _result[index]['navigate_to'], arguments: {'id': _result[index]['id']});
                },
              ),
            ),
          ),
              ...
              children: <Widget>[
                ...
                Container(
                  child: IconButton(
                      ...
                      iconSize: 20.0,
                      onPressed: () {
                        _cancleFavorite(_result[index]['id'], _result[index]['navigate_to'], index);
                      }),
                ),
              ],
            ),
  ...
  Future _cancleFavorite(int id, String navigateTo, int index) async {
    var res = await toDelFavorite(params: {'id': id, 'navigateTo': navigateTo});
    if (null != res) {
      if (res['data'] == true) {
