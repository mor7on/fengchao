...
class EditTeamComponent extends StatelessWidget {
  const EditTeamComponent({Key key}) : super(key: key);
...
class EditTeamForm extends StatefulWidget {
  EditTeamForm({Key key}) : super(key: key);
...
class _EditTeamFormState extends State<EditTeamForm> {
  ...
  void _initUserData() {
    _model = Provider.of<LoginUserModel>(context, listen: false).userTeam;
    _cateIds = [];
    _postStatus = _model.postStatus;
    _tagsController.text = _model.postKeywords;
  }
  ...
  Future _doPublish() async {
    Map<String, dynamic> res = await editTeam(params: {
      'id': _model.id,
      'post_title': _title,
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Padding(
              padding: EdgeInsets.fromLTRB(20.0, 8.0, 20.0, 8.0),
              child: TextFormField(
                initialValue: _model.postContent,
                decoration: const InputDecoration(
                  border: UnderlineInputBorder(),
