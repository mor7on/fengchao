...
class UserDynamicComponent extends StatefulWidget {
  UserDynamicComponent({Key key, this.arguments}) : super(key: key);

  final Map<String, dynamic> arguments;

  _UserDynamicComponentState createState() => _UserDynamicComponentState();
}

class _UserDynamicComponentState extends State<UserDynamicComponent> {
  int page = 0;
  ...
  initUserData() async {
    await Future.delayed(Duration(milliseconds: 600), () async {
      var res = await getUserShareList(params: {'user_id': 3, 'page': 0});

      if (null != res) {
        _result = [];
        for (var item in res['data']) {
          _result.add(MineShare.fromJson(item));
        }
      }
    });
    if (mounted) {
    ...
    return SmartRefresher(
      ...
      onRefresh: () async {
        ...
        await Future.delayed(Duration(milliseconds: 1200), () {
          getMyShareList(params: requestParams).then((v) {
            if (null != v) {
              if (v.length > 0) {
                setState(() {
                  _result = v;
          ...
          child: Row(
            children: <Widget>[
              Container(
                width: 60.0,
                alignment: Alignment.center,
                child: Text(lastUserShareTime != null &&
              ...
              userShare.imageList.length == 0
                  ...
                  : Container(
                      ...
                      child: Image.network(userShare.imageList[0]),
                    ),
              Expanded(
                child: Container(
                  width: double.infinity,
