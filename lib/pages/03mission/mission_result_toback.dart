...
class MissionResultToBackComponent extends StatelessWidget {
  const MissionResultToBackComponent({Key key, this.arguments}) : super(key: key);
  final Map arguments;
  @override
      ...
      body: GestureDetector(
        ...
        child: CustomExpandScrollView(
          child: Container(
            ...
            child: MissionResultToBackFormList(taskInfo: arguments),
          ),
        ),
      ),
    );
...
class MissionResultToBackFormList extends StatefulWidget {
  MissionResultToBackFormList({Key key, this.taskInfo}) : super(key: key);
  final Map taskInfo;

  _MissionResultToBackFormListState createState() => _MissionResultToBackFormListState();
}

class _MissionResultToBackFormListState extends State<MissionResultToBackFormList> {
  ...
  String _validateContent(String value) {
    if (value.isEmpty) return '退回原因不能为空';
    return null;
  }
  ...
  Future _doPublish() async {
    print(widget.taskInfo['trade_id']);
    more = jsonEncode({'photos': _imageUrls});
    Map<String, dynamic> postParams = {"trade_id": widget.taskInfo['trade_id'], "content": content, "more": more};
    Map<String, dynamic> res = await toRefusResultById(params: postParams);
    print(res);
    if (null != res) {
      if (res['data'] == true) {
          ...
          Padding(
            ...
            child: TextFormField(
              decoration: const InputDecoration(
                ...
                hintText: '退回原因的简要说明（例如：什么工作内容不满意...）',
                helperText: '请简要描述您的退回原因，可适当配以图片说明。',
                labelText: '退回原因描述',
                isDense: true,
