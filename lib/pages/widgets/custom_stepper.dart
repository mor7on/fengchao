...
class _CustomStepperState extends State<CustomStepper> with TickerProviderStateMixin {
  ...
  Color _circleColor(int index) {
    ...
    if (!_isDark()) {
      return widget.steps[index].isActive ? themeData.primaryColor : widget.steps[index].state == StepState.complete ? Colors.green[200] : Colors.black38;
    } else {
      return widget.steps[index].isActive ? themeData.accentColor : widget.steps[index].state == StepState.complete ? Colors.green[200] : themeData.backgroundColor;
    }
  }
  ...
  Widget _buildVerticalContents(index) {
    return Container(
      width: double.infinity,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: widget.steps[index].content.length == 0
        ? [Container()]
        : widget.steps[index].content.map((item)=>item).toList(),
      )
    );
  }
    ...
    return Stack(
      children: <Widget>[
        ...
        AnimatedCrossFade(
          ...
          secondCurve: const Interval(0.4, 1.0, curve: Curves.fastOutSlowIn),
          sizeCurve: Curves.fastOutSlowIn,
          crossFadeState: true ? CrossFadeState.showSecond : CrossFadeState.showFirst,
          duration: kThemeAnimationDuration,
        ),
      ],
    );
          ...
          child: ListView(
            ...
            children: <Widget>[
              AnimatedSize(
                ...
                vsync: this,
                child: Column(
                  children: widget.steps[widget.currentStep].content.map((f)=>f).toList(),
                ),
              ),
              // _buildVerticalControls(),
            ],
