...
class CompanyDetailComponent extends StatefulWidget {
  final Map arguments;

  CompanyDetailComponent({Key key, this.arguments}) : super(key: key);

  _CompanyDetailComponentState createState() => _CompanyDetailComponentState();
}

class _CompanyDetailComponentState extends State<CompanyDetailComponent> {
  ArticleModel detail;
  ...
  initUserData() async {
    ...
    await Future.delayed(Duration(milliseconds: 200), () async {
      var res = await getCompanyById(params: widget.arguments);
      if (null != res) {
        detail = ArticleModel.fromMap(res['data']);
  ...
  Future _navigateToEdit() async {
    Navigator.pushNamed(context, '/companyEdit', arguments: {'detail': detail}).then((isRefresh) {
      if (isRefresh == true) {
        initUserData();
      }
    });
  ...
  Future _toFavorite() async {
    var res = await toFavoriteCompany(params: {'id':detail.id});
    print(res);
    if (null != res) {
      if (res['data'] == 1) {
  ...
  Future _toDelete() async {
    var res = await deleteCompany(params: {'id': detail.id});
    if (null != res) {
      if (res['data'] == true) {
  ...
  void _toPublishReply(val) async {
    Map<String, dynamic> data = {
      "company_id": widget.arguments['id'],
      "content": val,
      "discuss_id": _discussId,
      "parent_id": _parentId,
    };
    var res = await toCompanyReply(params: data);
    print(res);
    if (null != res) {
      _comments.add(
        Comment(
