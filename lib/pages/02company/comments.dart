...
class CompanyComments extends StatefulWidget {
  CompanyComments({Key key, this.arguments}) : super(key: key);
  final Map arguments;
...
class _CompanyCommentsState extends State<CompanyComments> {
  ...
  void initUserData() async {
    ...
    var res = await getCompanyComments(params: {'date': date, 'company_id': widget.arguments['id'], 'page': page});
    if (null != res) {
      _aComments = [];
  ...
  void _onLoading() async {
    ...
    print(requestParams);
    await getCompanyComments(params: requestParams).then((v) {
      if (null != v) {
  ...
  void _onRefresh() async {
    ...
    await Future.delayed(Duration(milliseconds: 1200), () {
      getCompanyComments(params: requestParams).then((v) {
        if (null != v) {
          // if (v.length > 0) {
          //   setState(() {
          //     _result = v;
